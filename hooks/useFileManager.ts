...
export const useFileManager = (initialFiles: NewFile[], userId: string) => {
  ...
  const handleFileUpload = useCallback(async (uploadFiles: FileList, currentFolderId: string | null) => {
    ...
    const overallToast = toast.loading(`Starting upload of ${filesToUpload.length} file(s)...`);
    ...
    try {
      ...
      // Update the final toast based on how many uploads actually succeeded
      if (successfulUploads.length === filesToUpload.length) {
        toast.success(`Successfully uploaded ${successfulUploads.length} file(s).`, { id: overallToast });
      } else if (successfulUploads.length > 0) {
        toast.warning(`Completed with errors. ${successfulUploads.length} of ${filesToUpload.length} files uploaded.`, { id: overallToast });
      } else {
        toast.error('All uploads failed. Check the console for details.', { id: overallToast });
      }

    } catch (error) {
      // This catch block might not be strictly necessary anymore, but it's good for safety
      toast.error('A critical error occurred during the upload process.', { id: overallToast });
    }
  ...
  const toggleStar = useCallback(async (fileIds: string[]) => {
    ...
    // Assume all items in a bulk operation will be un-starred if the first is.
    const isStarring = !firstItem.isStarred;
    const itemType = firstItem.isFolder ? 'Folder' : 'File';

    const idsToToggle = new Set(fileIds);
    setFiles(prev => prev.map(f => f.id && idsToToggle.has(f.id) ? { ...f, isStarred: !f.isStarred } : f));

    const toastMessage = isBulk
      ? `${isStarring ? 'Starring' : 'Unstarring'} ${fileIds.length} items...`
      : `${isStarring ? 'Starring' : 'Unstarring'} ${itemType}...`;
    ...
    toast.promise(
      ...
      )),
      {
        ...
        success: isBulk ? `Updated ${fileIds.length} items.` : `${itemType} ${isStarring ? 'starred' : 'unstarred'}.`,
