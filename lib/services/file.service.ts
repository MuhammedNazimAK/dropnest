...
export async function moveFileService(
    ...
    if (!fileToMove.isFolder && fileToMove.fileIdInImageKit) {
        ...
        const copyResponse = await imageKitClient.upload({
            file: fileToMove.fileUrl,
            fileName: fileToMove.name,
            folder: newImageKitFolderPath.replace(/\/$/, ''),
            useUniqueFileName: false, // Keep the exact name
        });

        await imageKitClient.deleteFile(fileToMove.fileIdInImageKit);

        // Update the object with the new data for the transaction
        fileToMove.fileIdInImageKit = copyResponse.fileId;
        fileToMove.fileUrl = copyResponse.url;
        fileToMove.thumbnailUrl = copyResponse.thumbnailUrl;
        fileToMove.path = copyResponse.filePath;
    }
